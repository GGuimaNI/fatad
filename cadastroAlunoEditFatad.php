                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        ...
                        if ($stmt->rowCount() > 0) {
                            ...
                            echo "<script>
                                    alert('CPF: " . htmlspecialchars($cpfAluno, ENT_QUOTES, 'UTF-8'). " ainda não foi cadastrado como aluno.');
                                    window.location.href = 'cadastroListVisitantes.php';
                                </script>";
                                exit(); 
                        ...
                        ?>
                            ...
                            <div class="mb-3">
                                <label>CPF:</label>
                                <?php $cpfLimpo = isset($aluno['cpfAluno']) ? preg_replace('/[^0-9]/', '', $aluno['cpfAluno']) : ''; ?>
                                <input type="number" name="cpfAluno" 
                                value="<?= htmlspecialchars($cpfLimpo) ?>" class="form-control" readonly>                            
                            </div> 
                            <div class="mb-3">
                                <label>Identidade:</label>
                                <input type="text" placeholder="Apenas números" name="idtAluno" id="idtAluno" value="<?= isset($aluno['idtAluno']) ? htmlspecialchars($aluno['idtAluno']) : '' ?>" class="form-control">
                            </div>   

                            <div class="mb-3">
                                <label>Natural de:</label>
                                <input type="text" placeholder="Insira Cidade-UF" name="cidadeNatal" id="cidadeNatal" value="<?= isset($aluno['cidadeNatAluno']) ? htmlspecialchars($aluno['cidadeNatAluno']) : '' ?>" class="form-control">
                            </div>
                            <div class="mb-3">
                                <label>Data de Nascimento:</label>
                                <input type="date" required="" placeholder="dd/mm/AAAA" name="dtNasc" id="dtNasc" 
                                value="<?= isset($aluno['dtNascAluno']) && !empty($aluno['dtNascAluno']) ? date('Y-m-d', strtotime($aluno['dtNascAluno'])) : '' ?>" 
                                class="form-control">                            
                            </div>  
                            <div class="mb-3">
                            ...
                            <div class="mb-3">
                                ...
                                <input type="text" placeholder="Nome da mãe completo" name="nomeMae" id="nomeMaeAluno" value="<?= isset($aluno['nomeMaeAluno']) ? htmlspecialchars($aluno['nomeMaeAluno']) : '' ?>" class="form-control">
                            </div> 
                            ...
                            <div class="mb-3">
                                <label>E-mail:</label>
                                <input type="email" name="emailAluno" value="<?= isset($aluno['emailAluno']) ? htmlspecialchars($aluno['emailAluno']) : '' ?>" class="form-control" readonly>
                            </div>  
                            <div class="mb-3">
                                <label>Telefone:</label>
                                <input type="tel"    name="telZap" value="<?= isset($aluno['telZapAluno']) ? htmlspecialchars($aluno['telZapAluno']) : '' ?>" class="form-control" readonly>

                            </div> 

                            <div class="mb-3">
                                <label class="form-label">CEP:</label>
                                <input type="text"  value="<?= isset($aluno['cep']) ? htmlspecialchars($aluno['cep']) : '' ?>" name="cep" id="cep" class="form-control" class="form-control">
                            </div>
                            <div class="mb-3">
                                <label class="form-label">Endereço:</label>
                                <input type="text"  name="enderecoAluno" id="enderecoAluno" value="<?= isset($aluno['enderecoAluno']) ? htmlspecialchars($aluno['enderecoAluno']) : '' ?>"  class="form-control">
                            </div>
                           
                            <div class="mb-3">
                                <label class="form-label">Cidade-UF:</label>
                                <input type="text" name="cidadeUF" value="<?= isset($aluno['cidadeMoradia']) ? htmlspecialchars($aluno['cidadeMoradia']) : '' ?>" class="form-control">
                            </div>
                            ...
                            <div class="mb-3">
                                ...
                                <label>
                                    <input type="radio" required="" name="escolaridade" value="Fundamental" <?= ($escolaridade == 'Fundamental') ? 'checked' : '' ?>> Fundamental
                                </label>
                                <label>
                            ...
                            <div class="mb-3">
                              <button type="submit" name="updateAlunoEditFatad" class="btn btn-primary">Salvar</button>
                            </div>
